...
func ParseTimeTz(inputStr string, defaultTimezoneStr string) (time.Time, error) {
	...
	if err == nil {
		...
		if offsetSeconds > MaxOffsetSeconds || offsetSeconds < MinOffsetSeconds {
			...
			return time.Time{}, merr.WrapErrParameterInvalidMsg("UTC offset hour %d is out of the valid range [%d, %d]", offsetHours, MinOffsetSeconds/3600, MaxOffsetSeconds/3600)
		}
