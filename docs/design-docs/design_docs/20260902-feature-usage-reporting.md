...
- **Approver(s):** TBD
- **Status:** Draft
- **Component:** MixCoord, Proxy, DataCoord (index and segment metadata), DataNode (import / compaction counters), internal proto
...
| `pkg/proto/data_coord.proto` | `DataNode` | `rpc GetFeatureUsage(internal.GetFeatureUsageRequest) returns (internal.GetFeatureUsageResponse)` — one node's view: the import-file-type and compaction-type counters |
...
`QueryNode` does not get the per-node RPC: it has no entries to return yet, and an RPC that returns an
empty list on every node would be dead code until it does.
...
| `request` | Proxy, DataNode | monotonic count of uses of this feature since `node_start_time`; `last_used_at` is the unix time of the most recent hit, `0` if never hit in this process. Each counter is tagged with the role that owns it and appears only in that role's response |
...
Counters are **per node**. The report lists one `FeatureUsageNode` per Proxy and per DataNode; MixCoord
does not merge them, so the consumer can apply per-node reasoning (restart detection, node disappearance)
before summing. Every counter carries the role that owns it (Proxy for request-level features, DataNode
for import file types and compaction types), and a node's `GetFeatureUsage` returns only its role's
...
| Import file type, compaction type (DataNode) | where the import task opens each file (`importv2/task_import.go`) and where the compaction executor starts a task (`compactor/executor.go`) |
| Not yet hooked (catalog rows recommended but unimplemented) | `group_size` / `strict_group_size`, `rank_group_scorer`, `group_by_fields` (query), `ignore_growing`, `hints`, `analyzer_name`, `output_dynamic_field`, `norm_score`, `fragment_size` / `num_of_fragments`. Each is one branch at a parse site that already exists; they wait for the product selection |
...
present". Rows marked "yes" without qualification are implemented on the implementation branch; rows
marked "not yet hooked" are recommended but wait for the product selection (see "Local Verification" for
which implemented rows were also exercised end to end).
...
| `group_size` / `strict_group_size` | key present | yes — **not yet hooked** |
| `rank_group_scorer` | key present | yes — **not yet hooked** |
| `group_by_fields` (query path) | key present | yes — **not yet hooked** |
...
| `ignore_growing` | `== true` | yes — **not yet hooked** |
| `hints` | key present | yes — **not yet hooked** |
| `analyzer_name` | key present | yes — **not yet hooked** |
| `search_by_primary_keys` | field `true` | yes |
| `namespace` | field set | yes |
| `output_dynamic_field` (named) | `translateOutputFields` resolved a dynamic field | yes — **not yet hooked** |
...
| `norm_score` | key present in `rank_params` | yes — **not yet hooked** |
...
| `fragment_size` / `num_of_fragments` | key present in highlighter params | optional — not yet hooked |
