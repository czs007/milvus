...
func (reducer *GroupAggReducer) EmptyAggResult() (*AggregationResult, error) {
	...
	for _, agg := range reducer.aggregates {
		if agg.GetOp() == planpb.AggregateOp_count {
			...
			if err != nil {
				return nil, merr.WrapErrServiceInternalErr(err, fmt.Sprintf("failed to get field schema for aggregate fieldID %d", agg.GetFieldId()))
			}
			resultType, err := getAggregateResultType(agg.GetOp(), field.GetDataType())
			if err != nil {
				return nil, merr.WrapErrServiceInternalErr(err, fmt.Sprintf("failed to get result type for aggregate fieldID %d", agg.GetFieldId()))
			}
			emptyFieldData, err := genEmptyFieldDataByType(resultType)
			if err != nil {
				return nil, merr.WrapErrServiceInternalErr(err, fmt.Sprintf("failed to generate empty field data for result type %s", resultType.String()))
			}
			ret.fieldDatas = append(ret.fieldDatas, emptyFieldData)
		}
	}
...
func genEmptyFieldDataByType(dataType schemapb.DataType) (*schemapb.FieldData, error) {
	...
	default:
		...
		return nil, merr.WrapErrOperationNotSupportedMsg(fmt.Sprintf("unsupported data type for aggregate result: %s", dataType.String()))
	}
}
...
func getAggregateResultType(op planpb.AggregateOp, inputType schemapb.DataType) (schemapb.DataType, error) {
	...
	case planpb.AggregateOp_sum:
		...
		case schemapb.DataType_Float, schemapb.DataType_Double:
			return schemapb.DataType_Double, nil
		default:
			return schemapb.DataType_None, merr.WrapErrOperationNotSupportedMsg(fmt.Sprintf("unsupported input type %s for sum aggregation", inputType.String()))
		}
	default:
		return schemapb.DataType_None, merr.WrapErrOperationNotSupportedMsg(fmt.Sprintf("unknown aggregate operator: %d", op))
	}
}
...
func (reducer *GroupAggReducer) validateAggregationResults(results []*AggregationResult) error {
	if reducer.schema == nil {
		return merr.WrapErrServiceInternalMsg("schema is nil, cannot validate field types")
	}

	helper, err := typeutil.CreateSchemaHelper(reducer.schema)
	if err != nil {
		return merr.WrapErrServiceInternalErr(err, "failed to create schema helper")
	}
	...
	for _, fieldID := range reducer.groupByFieldIds {
		field, err := helper.GetFieldFromID(fieldID)
		if err != nil {
			return merr.WrapErrServiceInternalErr(err, fmt.Sprintf("failed to get field schema for groupBy fieldID %d", fieldID))
		}
	...
	for _, agg := range reducer.aggregates {
		...
		if agg.GetOp() == planpb.AggregateOp_count {
			...
			field, err := helper.GetFieldFromID(agg.GetFieldId())
			if err != nil {
				return merr.WrapErrServiceInternalErr(err, fmt.Sprintf("failed to get field schema for aggregate fieldID %d", agg.GetFieldId()))
			}
			expectedType, err = getAggregateResultType(agg.GetOp(), field.GetDataType())
			if err != nil {
				return merr.WrapErrServiceInternalErr(err, fmt.Sprintf("failed to get aggregate result type for aggregate fieldID %d", agg.GetFieldId()))
			}
		}
	...
	for resultIdx, result := range results {
		if result == nil {
			return merr.WrapErrParameterInvalidMsg(fmt.Sprintf("result at index %d is nil", resultIdx))
		}
		...
		if len(fieldDatas) != expectedColumnCount {
			return merr.WrapErrParameterInvalidMsg(fmt.Sprintf("result at index %d has fieldDatas length %d, expected %d (numGroupingKeys=%d, numAggs=%d)",
		...
		for colIdx, fieldData := range fieldDatas {
			if fieldData == nil {
				return merr.WrapErrParameterInvalidMsg(fmt.Sprintf("result at index %d has nil fieldData at column %d", resultIdx, colIdx))
			}

			expectedType := expectedTypes[colIdx]
			actualType := fieldData.GetType()
			if actualType != expectedType {
				return merr.WrapErrParameterInvalidMsg(fmt.Sprintf("result at index %d, column %d has type %s, expected %s",
...
func (reducer *GroupAggReducer) Reduce(ctx context.Context, results []*AggregationResult) (*AggregationResult, error) {
	...
	for _, result := range results {
		if result == nil {
			return nil, merr.WrapErrParameterInvalidMsg("input result from any sources cannot be nil")
		}
		reducedResult.allRetrieveCount += result.GetAllRetrieveCount()
		fieldDatas := result.GetFieldDatas()
		...
		for i := 0; i < outputColumnCount; i++ {
			...
			if rowCount == -1 {
				...
				if rowCount != hashers[i].RowCount() {
					return nil, merr.WrapErrServiceInternalMsg(fmt.Sprintf("field data:%d for different columns have different row count, %d vs %d, wrong state",
...
func SegcoreResults2AggResult(results []*segcorepb.RetrieveResults) ([]*AggregationResult, error) {
	...
	for i := 0; i < len(results); i++ {
		if results[i] == nil {
			return nil, merr.WrapErrParameterInvalidMsg("input segcore query results from any sources cannot be nil")
		}
