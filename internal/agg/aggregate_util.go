...
func NewFieldAccessor(fieldType schemapb.DataType) (FieldAccessor, error) {
	...
	default:
		return nil, merr.WrapErrOperationNotSupportedMsg("unsupported data type for hasher")
	}
}
...
func AssembleSingleValue(fv *FieldValue, fieldData *schemapb.FieldData) error {
	...
	if isNull {
		...
		default:
			return merr.WrapErrOperationNotSupportedMsg(fmt.Sprintf("unsupported DataType:%d", fieldData.GetType()))
		}
		return nil
	}
	...
	case schemapb.DataType_Bool:
		boolVal, ok := val.(bool)
		if !ok {
			return merr.WrapErrServiceInternalMsg(fmt.Sprintf("type assertion failed: expected bool, got %T", val))
		}
		fieldData.GetScalars().GetBoolData().Data = append(fieldData.GetScalars().GetBoolData().GetData(), boolVal)
	case schemapb.DataType_Int8, schemapb.DataType_Int16, schemapb.DataType_Int32:
		intVal, ok := val.(int32)
		if !ok {
			return merr.WrapErrServiceInternalMsg(fmt.Sprintf("type assertion failed: expected int32, got %T", val))
		}
		fieldData.GetScalars().GetIntData().Data = append(fieldData.GetScalars().GetIntData().GetData(), intVal)
	case schemapb.DataType_Int64:
		int64Val, ok := val.(int64)
		if !ok {
			return merr.WrapErrServiceInternalMsg(fmt.Sprintf("type assertion failed: expected int64, got %T", val))
		}
		fieldData.GetScalars().GetLongData().Data = append(fieldData.GetScalars().GetLongData().GetData(), int64Val)
	case schemapb.DataType_Timestamptz:
		timestampVal, ok := val.(int64)
		if !ok {
			return merr.WrapErrServiceInternalMsg(fmt.Sprintf("type assertion failed: expected int64 for Timestamptz, got %T", val))
		}
	...
	case schemapb.DataType_Float:
		...
		if !ok {
			return merr.WrapErrServiceInternalMsg(fmt.Sprintf("type assertion failed: expected float32, got %T", val))
		}
		fieldData.GetScalars().GetFloatData().Data = append(fieldData.GetScalars().GetFloatData().GetData(), floatVal)
	case schemapb.DataType_Double:
		doubleVal, ok := val.(float64)
		if !ok {
			return merr.WrapErrServiceInternalMsg(fmt.Sprintf("type assertion failed: expected float64, got %T", val))
		}
		fieldData.GetScalars().GetDoubleData().Data = append(fieldData.GetScalars().GetDoubleData().GetData(), doubleVal)
	case schemapb.DataType_VarChar, schemapb.DataType_String:
		stringVal, ok := val.(string)
		if !ok {
			return merr.WrapErrServiceInternalMsg(fmt.Sprintf("type assertion failed: expected string, got %T", val))
		}
...
func ComputeAvgFromSumAndCount(sumFieldData *schemapb.FieldData, countFieldData *schemapb.FieldData) (*schemapb.FieldData, error) {
	if sumFieldData == nil || countFieldData == nil {
		return nil, merr.WrapErrParameterInvalidMsg("sumFieldData and countFieldData cannot be nil")
	}
	...
	if countType != schemapb.DataType_Int64 {
		return nil, merr.WrapErrParameterInvalidMsg(fmt.Sprintf("count field must be Int64 type, got %s", countType.String()))
	}
	...
	case schemapb.DataType_Int64:
		...
		if len(sumData) != rowCount {
			return nil, merr.WrapErrParameterInvalidMsg(fmt.Sprintf("sum and count field data must have the same length, got sum:%d, count:%d", len(sumData), rowCount))
		}
		for i := 0; i < rowCount; i++ {
			if countData[i] == 0 {
				return nil, merr.WrapErrParameterInvalidMsg(fmt.Sprintf("division by zero: count is 0 at row %d", i))
			}
	...
	default:
		return nil, merr.WrapErrParameterInvalidMsg(fmt.Sprintf("unsupported sum field type for avg computation: %s", sumType.String()))
	}
