...
func NewAggregate(aggregateName string, aggFieldID int64, originalName string, fieldType schemapb.DataType) ([]AggregateBase, error) {
	if !isSupportedAggregateName(aggregateName) {
		return nil, merr.WrapErrParameterInvalidMsg(fmt.Sprintf("invalid Aggregation operator %s", aggregateName))
	}
...
func FromPB(pb *planpb.Aggregate) (AggregateBase, error) {
	...
	default:
		return nil, merr.WrapErrParameterInvalidMsg(fmt.Sprintf("invalid Aggregation operator %d", pb.Op))
	}
}
...
func AccumulateFieldValue(target *FieldValue, new *FieldValue) error {
	...
	if reflect.TypeOf(target.val) != reflect.TypeOf(new.val) {
		return merr.WrapErrServiceInternalMsg(fmt.Sprintf("type mismatch: target is %T, new is %T", target.val, new.val))
	}
	...
	default:
		return merr.WrapErrServiceInternalMsg(fmt.Sprintf("unsupported type: %T", target.val))
	}
	return nil
}
...
func (bucket *Bucket) Accumulate(row *Row, idx int, keyCount int, aggs []AggregateBase) error {
	if idx >= len(bucket.rows) || idx < 0 {
		return merr.WrapErrParameterInvalidMsg(fmt.Sprintf("wrong idx:%d for bucket", idx))
	}
	targetRow := bucket.rows[idx]
	if targetRow == nil {
		return merr.WrapErrServiceInternalMsg(fmt.Sprintf("nil row at the target idx:%d, cannot accumulate the row", idx))
	}
	if row.ColumnCount() != targetRow.ColumnCount() {
		return merr.WrapErrParameterInvalidMsg(fmt.Sprintf("column count:%d in the row must be equal to the target row:%d", row.ColumnCount(), bucket.rows[idx].ColumnCount()))
	}
	if row.ColumnCount() != keyCount+len(aggs) {
		return merr.WrapErrParameterInvalidMsg(fmt.Sprintf("column count:%d in the row must be sum of keyCount:%d and the number of aggs:%d", row.ColumnCount(), keyCount, len(aggs)))
	}
