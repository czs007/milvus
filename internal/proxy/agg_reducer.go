...
func (reducer *MilvusAggReducer) Reduce(results []*internalpb.RetrieveResults) (*milvuspb.QueryResults, error) {
	...
	for i := 0; i < fieldCount; i++ {
		...
		if len(indices) == 0 {
			return nil, merr.WrapErrServiceInternalMsg("no indices found for output field at index %d", i)
			...
			if err != nil {
				return nil, merr.WrapErrServiceInternalErr(err, "failed to compute avg for field %s", reducer.outputMap.NameAt(i))
			}
			avgFieldData.FieldName = reducer.outputMap.NameAt(i)
			reOrganizedFieldDatas[i] = avgFieldData
		} else {
			return nil, merr.WrapErrServiceInternalMsg("unexpected number of indices (%d) for output field at index %d, expected 1 or 2", len(indices), i)
		}
	}
