...
// if removePkField is true, pk field will not be included in the first(resultFieldNames)/second(userOutputFields)
// return value.
	...
	for _, outputFieldName := range outputFields {
		...
		if outputFieldName == "*" {
			...
			if field, ok := allFieldNameMap[outputFieldName]; ok {
				if !schema.CanRetrieveRawFieldData(field) {
					return nil, nil, nil, false, merr.WrapErrServiceInternalMsg("not allowed to retrieve raw data of field %s", outputFieldName)
				}
	...
	for _, fieldData := range insertMsg.FieldsData {
		...
		for row, data := range strData.GetData() {
			...
			if !ok {
				...
				return merr.WrapErrServiceInternalMsg("input with analyzer should be utf-8 format, but row: %d not utf-8 format. data: %s", row, data)
			}
		}
	}
	return nil
