...
func (pw *PackedManifestRecordWriter) writeStatsV3() error {
	...
	if statsBlob != nil {
		...
		if err := packed.WriteFile(pw.storageConfig, fullPath, statsBlob.Value); err != nil {
			return merr.WrapErrStorage(err, "writeStatsV3: failed to write bloom filter stats")
		}
		...
		if err != nil {
			return merr.WrapErrStorage(err, "writeStatsV3: failed to add bloom filter stats to manifest")
		}
	...
	if len(bm25Blobs) > 0 {
		...
		for fieldID, blob := range bm25Blobs {
			...
			if err := packed.WriteFile(pw.storageConfig, fullPath, blob.Value); err != nil {
				return merr.WrapErrStorage(err, "writeStatsV3: failed to write bm25 stats")
			}
		...
		if err != nil {
			return merr.WrapErrStorage(err, "writeStatsV3: failed to add bm25 stats to manifest")
		}
		pw.manifest = newManifest
