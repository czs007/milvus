...
func ConvertToArrowSchema(schema *schemapb.CollectionSchema, useFieldID bool) (*arrow.Schema, error) {
	...
	appendArrowField := func(field *schemapb.FieldSchema) error {
		if serdeMap[field.DataType].arrowType == nil {
			return merr.WrapErrServiceInternal(fmt.Sprintf("unknown field data type [%s] for field [%s]", field.DataType, field.GetName()))
		}
		var dim int
		...
		case schemapb.DataType_BinaryVector, schemapb.DataType_Float16Vector, schemapb.DataType_BFloat16Vector,
			...
			if err != nil {
				return merr.WrapErrServiceInternal(fmt.Sprintf("dim not found in field [%s] params", field.GetName()))
			}
		default:
			dim = 0
		}
