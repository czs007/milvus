...
func (s *searchTask) searchOnStreaming() error {
	...
	_, err := s.QS.streaming.replica.getCollectionByID(s.CollectionID)
	if err != nil {
		return err
	}
...
func newSearchTask(ctx context.Context, src *querypb.SearchRequest) (*searchTask, error) {
	...
	if target.iReq.GetSerializedExprPlan() != nil {
		target.plan = &planpb.PlanNode{}
		err := proto.Unmarshal(target.iReq.GetSerializedExprPlan(), target.plan)
		if err != nil {
			return nil, err
		}
	}
