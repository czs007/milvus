...
func parseV3SegmentID(rootPath, filePath string) (int64, error) {
	if !strings.HasPrefix(filePath, rootPath) {
		return 0, merr.WrapErrParameterInvalidMsg("path %q does not contain rootPath %q", filePath, rootPath)
	}
	...
	if len(parts) < 5 || parts[0] != common.SegmentInsertLogPath {
		return 0, merr.WrapErrParameterInvalidMsg("not a V3 insert_log path: %s", filePath)
	}
