...
func getManifestSchemaByVersion(version int) (avro.Schema, error) {
	...
	default:
		return nil, merr.WrapErrParameterInvalidMsg("unsupported manifest schema version: %d", version)
	}
}
...
func (w *SnapshotWriter) Save(ctx context.Context, snapshot *SnapshotData) (string, error) {
	...
	if snapshot == nil {
		return "", merr.WrapErrParameterInvalidMsg("snapshot cannot be nil")
	}
	if snapshot.SnapshotInfo == nil {
		return "", merr.WrapErrParameterInvalidMsg("snapshot info cannot be nil")
	}
	collectionID := snapshot.SnapshotInfo.GetCollectionId()
	if collectionID <= 0 {
		return "", merr.WrapErrParameterInvalidMsg("invalid collection ID: %d", collectionID)
	}
	if snapshot.Collection == nil {
		return "", merr.WrapErrParameterInvalidMsg("collection description cannot be nil")
	}

	snapshotID := snapshot.SnapshotInfo.GetId()
	if snapshotID <= 0 {
		return "", merr.WrapErrParameterInvalidMsg("invalid snapshot ID: %d", snapshotID)
	}
...
func (w *SnapshotWriter) Drop(ctx context.Context, metadataFilePath string) error {
	...
	if metadataFilePath == "" {
		return merr.WrapErrParameterInvalidMsg("metadata file path cannot be empty")
	}
...
func (r *SnapshotReader) ReadSnapshot(ctx context.Context, metadataFilePath string, includeSegments bool) (*SnapshotData, error) {
	// Validate input
	if metadataFilePath == "" {
		return nil, merr.WrapErrParameterInvalidMsg("metadata file path cannot be empty")
	}
...
func (r *SnapshotReader) ListSnapshots(ctx context.Context, collectionID int64) ([]*datapb.SnapshotInfo, error) {
	// Validate input parameters
	if collectionID <= 0 {
		return nil, merr.WrapErrParameterInvalidMsg("invalid collection ID: %d", collectionID)
	}
