...
func estimateRowsBySegmentSize(segments []*SegmentView, expectedSegmentSize int64) (int64, error) {
	if expectedSegmentSize <= 0 {
		return 0, merr.WrapErrParameterInvalidMsg("invalid expected segment size %d", expectedSegmentSize)
	}
	...
	if totalRows == 0 || totalSize == 0 {
		return 0, merr.WrapErrCompactionResult("segment view does not contain size info to estimate row count")
	}

	rowSize := totalSize / float64(totalRows)
	if rowSize <= 0 {
		return 0, merr.WrapErrCompactionResult("invalid row size calculated from segment view")
	}

	rows := float64(expectedSegmentSize) / rowSize
	if rows <= 0 {
		return 0, merr.WrapErrCompactionResult("estimated max row count is not positive")
	}
