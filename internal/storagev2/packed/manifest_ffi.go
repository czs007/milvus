...
func CreateManifestForSegment(
	...
	if len(fragments) == 0 {
		return "", merr.WrapErrStorageMsg("fragments cannot be empty")
	}

	// Create column groups from fragments
	columnGroups, err := createColumnGroups(columns, format, fragments)
	if err != nil {
		return "", merr.WrapErrStorage(err, "failed to create column groups")
	}
...
func ReadFragmentsFromManifest(
	...
	if err != nil {
		return nil, merr.WrapErrServiceInternalErr(err, "failed to parse manifest path")
	}
