...
func convertElementTypeOfVectorArrayToArrowType(field *schemapb.FieldSchema) (arrow.DataType, error) {
	if field.GetDataType() != schemapb.DataType_ArrayOfVector {
		return nil, merr.WrapErrServiceInternal(fmt.Sprintf("field is not a vector array: %v", field.GetDataType().String()))
	}
	...
	default:
		return nil, merr.WrapErrServiceInternal(fmt.Sprintf("unsupported element type for ArrayOfVector: %v", field.GetElementType().String()))
	}
	return elemType, nil
}
...
func convertToArrowDataType(field *schemapb.FieldSchema, isArray bool) (arrow.DataType, error) {
	...
	default:
		return nil, merr.WrapErrServiceInternal(fmt.Sprintf("unsupported data type %v", dataType.String()))
	}
}
...
func ConvertToArrowSchemaForUT(schema *schemapb.CollectionSchema, useNullType bool) (*arrow.Schema, error) {
	...
	for _, structField := range schema.StructArrayFields {
		...
		for _, subField := range structField.Fields {
			...
			default:
				err = merr.WrapErrServiceInternal(fmt.Sprintf("unsupported data type in struct: %v", subField.DataType.String()))
			}
