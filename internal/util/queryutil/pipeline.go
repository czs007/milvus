...
func (n *Node) unpackInputs(msg OpMsg) ([]any, error) {
	...
	for i, input := range n.inputs {
		val, ok := msg[input]
		if !ok {
			return nil, merr.WrapErrServiceInternalMsg("node [%s]: input channel '%s' not found", n.name, input)
		}
		inputs[i] = val
	}
	return inputs, nil
...
func (n *Node) packOutputs(outputs []any, msg OpMsg) error {
	if len(outputs) != len(n.outputs) {
		return merr.WrapErrServiceInternalMsg("node [%s]: output count mismatch, expected %d, got %d",
...
func (n *Node) Run(ctx context.Context, span trace.Span, msg OpMsg) error {
	...
	if err != nil {
		return merr.WrapErrServiceInternalErr(err, "node [%s] operator failed", n.name)
	}
...
func (p *Pipeline) Run(ctx context.Context, span trace.Span, initialMsg OpMsg) (OpMsg, error) {
	...
	for _, node := range p.nodes {
		if err := node.Run(ctx, span, msg); err != nil {
			return nil, merr.WrapErrServiceInternalErr(err, "pipeline [%s]", p.name)
		}
	}
