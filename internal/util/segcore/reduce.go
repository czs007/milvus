...
func NewStreamReducer(ctx context.Context,
	...
	if len(sliceNQs) != len(sliceTopKs) {
		return nil, merr.WrapErrServiceInternal(fmt.Sprintf(
			"unaligned sliceNQs(len=%d) and sliceTopKs(len=%d)", len(sliceNQs), len(sliceTopKs)))
	}
...
func ReduceSearchResultsAndFillData(ctx context.Context, plan *SearchPlan, searchResults []*SearchResult,
	...
	if len(sliceNQs) != len(sliceTopKs) {
		return nil, merr.WrapErrServiceInternal(fmt.Sprintf("unaligned sliceNQs(len=%d) and sliceTopKs(len=%d)", len(sliceNQs), len(sliceTopKs)))
	}
