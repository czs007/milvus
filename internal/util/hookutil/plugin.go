...
func LoadPlugin[T any](path string, symbol string) (T, error) {
	...
	if err != nil {
		return zero, merr.WrapErrServiceInternalErr(err, "fail to open plugin %s", path)
	}

	sym, err := p.Lookup(symbol)
	if err != nil {
		return zero, merr.WrapErrServiceInternalErr(err, "fail to find symbol %q in plugin %s", symbol, path)
	}
