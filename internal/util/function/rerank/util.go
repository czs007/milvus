...
func groupScore[T PKType](group *Group[T], scorerType string) (float32, error) {
	...
	case avgScorer:
		if len(group.idList) == 0 {
			return 0, merr.WrapErrServiceInternal(fmt.Sprintf(
				"input group for score must have at least one id, got %d, there must be sth wrong within code", len(group.idList)))
		}
		return group.sumScore / float32(len(group.idList)), nil
	default:
		return 0, merr.WrapErrServiceInternal(fmt.Sprintf("input group scorer type: %s is not supported!", scorerType))
	}
}
