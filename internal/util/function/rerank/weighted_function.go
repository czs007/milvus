	...
	for _, param := range funcSchema.Params {
		...
		case WeightsParamsKey:
			if err := json.Unmarshal([]byte(param.Value), &weights); err != nil {
				return nil, merr.WrapErrParameterInvalidMsg(fmt.Sprintf(
					"Parse %s param failed, weight should be []float, bug got: %s", WeightsParamsKey, param.Value))
			}
		...
		case NormScoreKey:
			if needNorm, err = strconv.ParseBool(param.Value); err != nil {
				return nil, merr.WrapErrParameterInvalidMsg(fmt.Sprintf("%s params must be true/false, bug got %s", NormScoreKey, param.Value))
			}
		}
	}
	if len(weights) == 0 {
