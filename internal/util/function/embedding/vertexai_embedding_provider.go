...
func getVertexAIJsonKey() ([]byte, error) {
	...
	if jsonKeyPath == "" {
		return nil, merr.WrapErrFunctionFailedMsg("VetexAI credentials file path is empty")
	}
	...
	if err != nil {
		return nil, merr.WrapErrFunctionFailed(err, "Vertexai: read credentials file failed")
	}
