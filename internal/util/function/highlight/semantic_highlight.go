	...
	for _, param := range params {
		...
		case queryKeyName:
			if err := json.Unmarshal([]byte(param.Value), &queries); err != nil {
				return nil, merr.WrapErrParameterInvalidErr(err, "Parse queries failed")
			}
		case inputFieldKeyName:
			if err := json.Unmarshal([]byte(param.Value), &inputFields); err != nil {
				return nil, merr.WrapErrParameterInvalidErr(err, "Parse input_field failed")
			}
		}
	}
...
func (highlight *SemanticHighlight) processOneQuery(ctx context.Context, query string, documents []string) ([][]string, [][]float32, error) {
	...
	if len(highlights) != len(documents) || len(scores) != len(documents) {
		return nil, nil, merr.WrapErrServiceInternalMsg("Highlights size must equal to documents size, but got highlights size [%d], scores size [%d], documents size [%d]", len(highlights), len(scores), len(documents))
	}
