...
func InsertEmpty(n *filterNode, c *Collection, msg *InsertMsg) error {
	if len(msg.GetTimestamps()) <= 0 {
		return merr.WrapErrServiceInternal("in Insert msg, the length of timestamp field is 0")
	}
	return nil
}
...
func InsertOutOfTarget(n *filterNode, c *Collection, msg *InsertMsg) error {
	if msg.GetCollectionID() != c.ID() {
		return merr.WrapErrServiceInternal(fmt.Sprintf("in Insert msg, collectionID:%v in msg not match to %v", msg.GetCollectionID(), c.ID()))
	}
...
func DeleteOutOfTarget(n *filterNode, c *Collection, msg *DeleteMsg) error {
	if msg.GetCollectionID() != c.ID() {
		return merr.WrapErrServiceInternal(fmt.Sprintf("in Delete msg, collectionID:%v in msg not match to %v", msg.GetCollectionID(), c.ID()))
	}
